...
class BaseOptions():
    ...
    def initialize(self, parser):
        ...
        # 基本参数
        parser.add_argument('--dataroot', default="/home/ljp/a.krabs/dataset/EUVP/Paired/",
                            help='图像路径（应具有子文件夹 trainA、trainB、valA、valB 等）')
        parser.add_argument('--name', type=str, default='krabs_net',
                            help='实验的名称。它决定将样本和模型存储在何处')
        parser.add_argument('--gpu_ids', type=str, default='0', help='GPU ID：例如 0 0,1,2， 0,2。使用 -1 表示 CPU')
        ...
        parser.add_argument('--dataset_mode', type=str, default='underwater',
                            help='选择数据集的加载方式。 [unaligned | aligned | single | colorization | underwater]')
        parser.add_argument('--direction', type=str, default='AtoB', help='AtoB 或 BtoA')
        parser.add_argument('--serial_batches', action='store_true',
                            help='如果为 true，则拍摄图像以进行批量处理，否则随机拍摄图像')
        parser.add_argument('--num_threads', default=4, type=int, help='# 用于加载数据的线程数量')
        parser.add_argument('--batch_size', type=int, default=4, help='输入的批量大小')
