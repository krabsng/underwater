...
def populate_train_list(images_path, mode='train'):
    ...
    image_list_lowlight = glob.glob(images_path + '*/trainA/*.jpg',recursive=True) + glob.glob(images_path + '*/trainA/*.JPEG',recursive=True)
...
class lowlight_loader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
            ...
            # GTImage
            data_highlight = Image.open(data_lowlight_path.replace('trainA', 'trainB'))
            # 确保图像大小一致
            data_lowlight = data_lowlight.resize((256, 256), Image.ANTIALIAS)
            data_highlight = data_highlight.resize((256,256), Image.ANTIALIAS)
            ...
            if self.normalize:
                ...
                input, gt = transform_input(data_lowlight), transform_gt(data_highlight)
                return input, gt
            else:
                data_lowlight, data_highlight = torch.from_numpy(data_lowlight).float(), torch.from_numpy(data_highlight).float()
